...
def validar_linha(row, regras):
    ...
    ORG_POR_FONTE = regras['ORG_POR_FONTE']
    PROGRAMA_OBRIG = regras['PROGRAMA']
    ...
    if rd == 'R':
        ...
        if programa != PROGRAMA_OBRIG:
            erros.append(f"Programa deve ser '{PROGRAMA_OBRIG}'")
...
if uploaded:
    try:
        ...
        df['Erro'] = df.apply(lambda r: validar_linha(r, regras), axis=1)
        progresso_atual += 1
