...
def detectar_ano(df):
    try:
        ...
        if anos:
            return int(sorted(anos)[-1])  # Ano mais recente
    except Exception:
        pass
...
# --- App Streamlit ---
st.set_page_config(page_title='Validador SNC-AP Turbo Finalíssimo 2026.9', layout='wide')
st.title('🛡️ Validador de Lançamentos SNC-AP Turbo Finalíssimo 2026.9')
...
if uploaded:
    try:
        ...
        ano_default = detectar_ano(df_original)
        st.success(f"Ficheiro '{uploaded.name}' carregado com sucesso! (Ano detetado: {ano_default})")
...
ano_validacao = st.sidebar.selectbox(
    ...
    [2025, 2026, 2027],
    index=[2025, 2026, 2027].index(ano_default) if ano_default in [2025, 2026, 2027] else 0,
    placeholder='Escolha o ano…'
)

if uploaded and ano_validacao:
