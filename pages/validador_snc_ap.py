...
if uploaded:
    ...
    df = pd.read_csv(
        ...
        skiprows=9, names=CABECALHOS, keep_default_na=False
    )
    ...
    df['Erro'] = ["; ".join(erros_por_linha[i]) if erros_por_linha[i] else "Sem erros" for i in range(n)]
    ...
    st.download_button(
        "⬇️ Descarregar Excel de output", buffer,
        file_name=fname,
